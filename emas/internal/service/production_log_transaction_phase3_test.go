...
func TestProductionLogRollsBackWhenSlotIsMissing(t *testing.T) {
	db := testutil.NewTestDB(t)
	...
	_, err := svc.LogProduction(dto.LogProductionRequest{
		SlotID:           "SLOT-P4-MISSING",
		StartTime:        now,
		EndTime:          now.Add(time.Hour),
		QuantityProduced: 5,
	})
	if err == nil {
		t.Fatal("LogProduction error = nil, want missing slot error")
	}
	var logCount int64
	if err := db.Model(&domain.ProductionLogs{}).Where("slot_id = ?", "SLOT-P4-MISSING").Count(&logCount).Error; err != nil {
		t.Fatalf("count production logs: %v", err)
	}
	if logCount != 0 {
		t.Fatalf("production logs persisted after missing slot = %d, want 0", logCount)
	}
}

func TestProductionLogRollsBackWhenProposalOutcomeUpdateFails(t *testing.T) {
	db := testutil.NewTestDB(t)
	now := time.Date(2026, 5, 18, 9, 0, 0, 0, time.UTC)
	job := domain.Job{
		JobID:         "JOB-P4-PLOG-PROP",
		ProductID:     "PROD-P4-PLOG-PROP",
		QuantityTotal: 10,
		Priority:      domain.JobPriorityMedium,
		Deadline:      now.Add(24 * time.Hour),
		Status:        domain.JobStatusScheduled,
	...
	step := domain.JobSteps{
		JobStepID:      "JSTEP-P4-PLOG-PROP",
		JobID:          job.JobID,
		StepID:         "STEP-P4-PLOG-PROP",
		StepSequence:   1,
		QuantityTarget: 10,
		Status:         domain.JobStepStatusScheduled,
	}
	slot := domain.JobStepScheduleSlots{
		SlotID:          "SLOT-P4-PLOG-PROP",
		JobStepID:       step.JobStepID,
		ProposalID:      "PROP-P4-PLOG",
		MachineID:       "M-P4-PLOG-PROP",
		ScheduledStart:  now,
		ScheduledEnd:    now.Add(time.Hour),
		QuantityPlanned: 10,
		Status:          domain.SlotStatusPlanned,
	...
	proposal := domain.AIProposal{
		...
		JobID:       job.JobID,
		Version:     1,
		Status:      domain.AIProposalStatusApplied,
		GeneratedAt: now,
		CreatedAt:   now,
		UpdatedAt:   now,
	}
	...
	if err := db.Exec(`CREATE TRIGGER phase4_fail_proposal_update BEFORE UPDATE ON ai_proposals BEGIN SELECT RAISE(FAIL, 'forced proposal update failure'); END;`).Error; err != nil {
		t.Fatalf("create trigger: %v", err)
	}
	defer db.Exec("DROP TRIGGER IF EXISTS phase4_fail_proposal_update")
	...
	svc := NewProductionLogService(
		...
		repository.NewJobStepRepository(db),
		repository.NewJobRepository(db),
		repository.NewAIProposalRepository(db),
		nil,
	)
	_, err := svc.LogProduction(dto.LogProductionRequest{
		SlotID:           slot.SlotID,
	...
	if err == nil {
		t.Fatal("LogProduction error = nil, want forced proposal update failure")
	}
	...
	if logCount != 0 {
		t.Fatalf("production logs persisted after proposal update failure = %d, want 0", logCount)
	}
	reloadedSlot, err := repository.NewJobSlotRepository(db).GetByID(slot.SlotID)
	if err != nil {
		t.Fatalf("reload slot: %v", err)
	}
	if reloadedSlot.Status != domain.SlotStatusPlanned {
		t.Fatalf("slot status after rollback = %s, want %s", reloadedSlot.Status, domain.SlotStatusPlanned)
	}
}
