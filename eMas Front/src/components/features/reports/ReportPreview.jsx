...
import { formatReportValue } from './reportValueFormatter'
...
const ReportPreview = ({
    ...
    const displayDateRange = dateRange ? formatReportValue(dateRange) : ''
    const rows = (() => {
        if (data) {
        ...
        <div className="lg:col-span-2 flex flex-col gap-6 bg-surface-1 border border-hairline p-6 rounded-xl">
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-ink-subtle dark:text-[#9ab4bc]">{displayDateRange || 'Last 7 days'}</p>
                </div>
                        ...
                        <p className="text-sm text-ink-muted dark:text-[#9ab4bc] leading-relaxed">
                            {data ? (
                                <>
                                    ...
                                    {displayDateRange ? ` - ${displayDateRange}` : ' - Last 7 days'}.
                                </>
                            ) : (
                                ...
                                <tbody>
                                    {tableRows.map((row, i) => (
                                        <tr key={i} className="border-b border-hairline last:border-b-0 hover:bg-surface-2 /40 transition-colors">
                                            {headers.map((h) => (
                                                <td key={h} className="p-3 text-ink">{formatReportValue(row[h])}</td>
                                            ))}
                                        </tr>
                                    ))}
                                </tbody>
