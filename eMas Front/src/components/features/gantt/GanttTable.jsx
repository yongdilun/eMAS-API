...
const GanttTable = ({ jobs = [], machines: machinesProp = [], selectedJobId: selectedJobIdProp, selectedSlot = null, onJobClick, isPreview = false }) => {
 ...
 className={`h-8 rounded-md px-3 text-button transition-colors ${
 zoomLevel === idx ? 'bg-primary text-on-primary' : 'text-ink-subtle hover:bg-surface-2 hover:text-ink'
 }`}
 >
 {level.label}
 </button>
 ...
 <div
 className={`flex items-center justify-center border-b border-hairline px-3 text-sm font-semibold ${
 isTodayDate ? 'bg-surface-3 text-ink' : 'bg-surface-1 text-ink-muted'
 }`}
 ...
 className={`absolute left-0 top-2 h-14 cursor-pointer overflow-visible rounded-lg transition-colors hover:ring-2 hover:ring-primary/40 hover:ring-offset-1 hover:ring-offset-canvas ${
 isSelectedStep
