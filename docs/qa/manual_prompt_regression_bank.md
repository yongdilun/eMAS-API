...
| ID | Prompt / flow | Expected deterministic behavior | Coverage |
|---|---|---|---|
| `response-document-phase18-machine-status-clean-answer` | `Show status for machine with machine id M-CNC-01` | The read-only status answer is composed from typed facts, not raw assistant markdown. It renders one concise machine-status answer with human labels, shows `M-CNC-01` and `running`, avoids duplicate answer blocks, avoids dump-style labels such as `Machineid`, `Machinename`, `Capacityperhour`, and forbids `done_all`, raw `**Success**`, approval UI, and mutation UI. | Covered: `factory-agent/tests/test_response_document_contract.py::test_machine_status_read_only_response_uses_typed_status_contract`; `FactoryAgentChatPanel.component.test.mjs::FactoryAgentChatPanel renders read-only machine status from typed status_result without raw markdown`; `final-response-quality.spec.js::RD-008 read-only status response renders one clean machine status contract`; `responseDocumentProbe.js::readOnlyStatusForbiddenProbeText` |

## Response Document Phase 19 RAG Question-Type Routing Regression

| ID | Prompt / flow | Expected deterministic behavior | Coverage |
|---|---|---|---|
| `response-document-phase19-loto-document-content-notification` | `According to the LOTO procedure, what notification is required before starting lockout` | Route as a document-content RAG/procedure question, not machine-specific procedure selection. Do not ask for machine ID. Do not render `No results` or `completed_answer` technical diagnostic. Final response should answer the notification requirement with source evidence when available. | Covered: `test_intent_splitter.py::test_phase19_loto_notification_questions_are_document_content_not_machine_selection`; `test_phase19_prompt_workflow_regression.py::test_phase19_document_content_loto_notification_route_matrix`; `test_phase19_prompt_workflow_regression.py::test_phase19_document_content_loto_prompt_workflow_returns_clean_response_document`; `test_route_to_execution_contract.py::test_loto_document_content_routes_to_rag_without_machine_id_clarification`; mocked browser semantic probe `final-response-quality.spec.js::RD-009 response_document LOTO document content notification answer does not ask for machine ID` |
| `response-document-phase19-loto-document-content-affected-employees` | `What does the LOTO procedure say about notifying affected employees?` | Same document-content behavior as above. This prevents overfitting only the exact notification prompt. | Covered: intent splitter, prompt workflow route matrix, and route-to-execution matrix |
| `response-document-phase19-loto-document-content-before-lockout` | `Before lockout, who needs to be notified according to LOTO?` | Route to document-content RAG/procedure with empty `missing_required_entities`; no machine-ID clarification. | Covered: intent splitter, prompt workflow route matrix, and route-to-execution matrix |
| `response-document-phase19-loto-document-content-lockout-tagout-requirements` | `What are the notification requirements before lockout/tagout?` | Route to document-content RAG/procedure with empty `missing_required_entities`; no machine-ID clarification. | Covered: intent splitter, prompt workflow route matrix, and route-to-execution matrix |
| `response-document-phase19-osha-loto-notification-policy` | `According to OSHA LOTO guidance, what notification is required before lockout?` | Route to safety-policy RAG with empty `missing_required_entities`; no live machine status or machine-ID clarification. | Covered: intent splitter, prompt workflow route matrix, route-to-execution matrix, and LOTO notification knowledge policy fallback |
| `response-document-phase19-generic-document-content-contract` | Maintenance instruction, quality procedure, SOP, and job instruction notification prompts | Prove the question-type contract is reusable beyond one LOTO prompt family. | Covered: `test_intent_splitter.py::test_phase19_document_content_question_type_is_generic_beyond_loto`; `test_phase19_prompt_workflow_regression.py::test_phase19_document_content_contract_is_not_loto_prompt_specific` |
| `response-document-phase19-machine-specific-loto-control` | `What LOTO procedure applies before working on the CNC machine?` | Preserve correct machine-specific behavior: ask for exact machine ID when the user asks which procedure applies to an unspecified machine. | Covered: adjacent route-control tests in `test_intent_splitter.py` and `test_phase19_prompt_workflow_regression.py`; existing SO-022 seeded browser coverage remains valid |
| `response-document-phase19-machine-status-control` | `What is the status of M-CNC-01?` | Preserve live machine-status routing. A machine mention in a status question must not be sent to document RAG. | Covered: adjacent route-control tests in `test_intent_splitter.py`, `test_phase19_prompt_workflow_regression.py`, and `test_route_to_execution_contract.py`; Phase 18 RD-008 status coverage remains valid |
...
## Response Document Phase 21 Backend Metadata Readiness

Phase 21 is a backend-readiness phase, not a prompt-volume phase. It prepares OpenAPI, generated tools, generated vocabulary, and tool documentation so the generic entity response-document contract can be implemented safely afterward.
...
These are regression-bank candidates for migrating existing machine/job behavior onto generic response-document contracts. Phase 23 should not claim broader entity diversity yet.

| Candidate ID | Prompt / flow class | Expected deterministic behavior | First useful coverage |
|---|---|---|---|
| `response-document-phase23-machine-status-generic-contract` | Existing machine status read, such as `Show status for machine with machine id M-CNC-01`. | Same clean visible answer as Phase 18, but response document uses `entity_status_v1` and frontend checks contract evidence rather than the machine id. | Covered: `factory-agent/tests/test_response_document_contract.py::test_machine_status_read_only_response_uses_typed_status_contract`; `FactoryAgentChatPanel.component.test.mjs`; RD-008 mocked browser proof asserts `entity_status_v1` backend/visible contract evidence. |
| `response-document-phase23-job-priority-business-change` | Existing RD-001/RD-002 job priority cascade. | Final result still summarizes the correct business changes, but business grouping comes from typed `business_change_v1` fields rather than priority prose or job id shape. | Covered: `factory-agent/tests/test_response_document_contract.py::test_final_completed_mutation_document_aggregates_all_approved_changes`; `FactoryAgentChatPanel.component.test.mjs`; RD-001/RD-002 mocked browser oracle asserts `business_change_v1`, typed group fields, and compact final visual quality. |
| `response-document-phase23-job-noop-generic-contract` | Existing RD-006/RD-007 no-op job mutation flows. | No-op groups continue to show `Not changed`, avoid approval for zero-match groups, and use the generic no-match/no-op contract. | Covered: `factory-agent/tests/test_response_document_contract.py::test_partial_noop_plus_valid_mutation_is_visible_before_approval_and_final`; `test_all_noop_mutation_completes_without_approval_or_fake_success`; RD-006/RD-007 mocked browser proof asserts `entity_agnostic_no_matching_records_v1`. |

## Response Document Phase 24 Entity Diversity Coverage

Phase 24 proves the generic contracts beyond jobs and machines. Add executable entries only when a safe deterministic fixture or backend path exists.

| Candidate ID | Prompt / flow class | Expected deterministic behavior | First useful coverage |
|---|---|---|---|
| `response-document-phase24-product-status-read` | Product status or product read result. | Uses typed entity/read or `entity_status_v1` evidence with product entity metadata, human labels, no approval UI, and no raw API dump labels. | Covered: `factory-agent/tests/test_response_document_contract.py::test_phase24_product_status_read_result_uses_entity_status_contract`. Browser fixture not added because no visible DOM divergence was introduced. |
| `response-document-phase24-material-inventory-read` | Material or inventory availability/status read. | Uses typed entity/read or `entity_status_v1` evidence with material/inventory entity metadata and clean human labels. | Backend response-document contract first; browser fixture if rendering can diverge. |
| `response-document-phase24-work-order-status` | Work order status read. | Uses typed entity/read or `entity_status_v1` evidence with work-order entity metadata and no machine/job special casing. | Backend response-document contract first; browser fixture if rendering can diverge. |
| `response-document-phase24-non-job-noop-mutation` | Safe mutation-selector no-op for a non-job entity. | Emits generic no-match/no-op evidence with `Not changed`, no approval for zero-match groups, no fake success, and no mutation/audit rows for the no-op group. | Backend response-document contract or fixture; browser only if display changes. |
| `response-document-phase24-non-job-partial-noop-valid-group` | Non-job mutation flow with one no-op group and one valid group, only if a safe write path exists. | Final response shows both changed and not-changed groups from typed fields; approval contains only valid proposed mutations. | Covered: `factory-agent/tests/test_response_document_contract.py::test_phase24_material_partial_noop_plus_valid_group_uses_generic_contracts` using a backend contract fixture; no broad non-job write behavior was enabled. |

## Response Document Phase 25 Hardcode Regression Guardrails

Phase 25 prevents product code and tests from drifting back to exact fixture ids, exact prompts, or entity-label branches.
...
Phase 26 is the real/seeded release proof after Phases 21-25 are green.
