...
version: alpha
name: Linear
description: "A dual-theme product-focused canvas that supports both a near-black mode (#010102) and a crisp light mode (#ffffff), with the signature Linear lavender-blue (#5e6ad2) used as the single chromatic accent. The system reads as software-craft documentation: dense, technical, and quietly luxurious. Display type is set in the Linear custom sans (SF Pro Display fallback) at 500–700 with measured negative tracking. The accent lavender appears on the brand mark, focus rings, and a few intentional CTAs — never decoratively. Page rhythm leans on product UI screenshots framed in panels rather than atmospheric color."
...
colors:
  ...
  # Light Theme
  light-ink: "#111827"
  light-ink-muted: "#374151"
  light-ink-subtle: "#6b7280"
  light-ink-tertiary: "#9ca3af"
  light-canvas: "#ffffff"
  light-surface-1: "#f5f6f6"
  light-surface-2: "#eaebeb"
  light-surface-3: "#e1e3e4"
  light-surface-4: "#d8dadb"
  light-hairline: "#e5e7eb"
  light-hairline-strong: "#d1d5db"
  light-hairline-tertiary: "#9ca3af"
  light-inverse-canvas: "#010102"
  light-inverse-surface-1: "#0f1011"
  light-inverse-surface-2: "#141516"
  light-inverse-ink: "#f7f8f8"
...
Linear's product canvas is a dual-theme system that balances a deep dark mode and a crisp light mode. In dark mode, `{colors.canvas}` is #010102, essentially pure black with a faint blue tint. In light mode, `{colors.light-canvas}` is pure #ffffff white. On top sits a four-step surface ladder (`surface-1` through `surface-4`) for cards, panels, and lifted tiles, structured with hairline borders.

The single chromatic accent is **Linear lavender-blue** `{colors.primary}` (#5e6ad2) — used on the brand mark, focus rings, and the primary CTA button across both themes. A lighter hover state (`{colors.primary-hover}` #828fff) and a focus-tinted variant (`{colors.primary-focus}` #5e69d1) extend the same hue. Linear avoids saturated greens, oranges, reds, etc. — the only semantic color is `{colors.semantic-success}` (#27a644) for status pills and the rare success indicator.
...
**Key Characteristics:**
- **Dual-theme product canvas** — Support for both dark (`#010102`) and light (`#ffffff`) surfaces.
...
- Light mode tokens have been extrapolated from the inverse-colors and standard semantic principles to support a dual-theme environment.
